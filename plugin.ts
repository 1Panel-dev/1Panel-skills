...
function createClientFromPluginConfig(config: PluginConfig): OnePanelClient {
  return new OnePanelClient({
    ...
    skipTlsVerify:
      config.skipTlsVerify ?? /^(1|true|yes)$/i.test(process.env.ONEPANEL_SKIP_TLS_VERIFY || ''),
  });
}
    ...
    api.registerTool(
      {
        ...
        description:
          'Query a 1Panel instance through signed API calls. Supports listing modules, listing actions, running a module action, or making a raw signed request.',
        parameters: {
          type: 'object',
          additionalProperties: false,
          properties: {
